...
export default function BirthdayTemplateWarm({ formData, message, id }: Props) {
  return (
    <div id={id} style={{
      width: "600px", height: "780px", position: "relative", overflow: "hidden",
      background: "linear-gradient(150deg, #880E4F 0%, #C62828 35%, #E65100 70%, #EF6C00 100%)",
      fontFamily: "'Segoe UI', Arial, sans-serif",
    }}>
      <svg style={{ position:"absolute", inset:0, width:"600px", height:"780px", pointerEvents:"none", zIndex:2 }}
        viewBox="0 0 600 780">
        <defs>
          <radialGradient id="w-bal" cx="35%" cy="30%" r="65%">
        ...
        {RECTS.map((p, i) => (
          <rect key={i} x={p.x-p.w/2} y={p.y-p.h/2} width={p.w} height={p.h}
            fill={i % 2 === 0 ? "#D4AF37" : "#FF8C00"} opacity={0.78}
        ...
        {/* ── Subtle warm glow ── */}
        <circle cx={300} cy={780} r={220} fill="rgba(255,140,0,0.10)" />

        {/* ── Left balloon ── */}
        <ellipse cx={46}  cy={600} rx={30} ry={40} fill="url(#w-bal)" opacity={0.90} />
        ...
        {/* ── Right balloon ── */}
        <ellipse cx={554} cy={600} rx={30} ry={40} fill="url(#w-bal)" opacity={0.90} />
        ...
        {/* ── Left gift box ── */}
        <rect x={38}  y={724} width={76} height={50} rx={4} fill="#7B1818" stroke="#D4AF37" strokeWidth={1.5} />
        <rect x={34}  y={708} width={84} height={18} rx={4} fill="#6B1010" stroke="#D4AF37" strokeWidth={1.5} />
        ...
        {/* ── Right gift box ── */}
        <rect x={486} y={724} width={76} height={50} rx={4} fill="#7B1818" stroke="#D4AF37" strokeWidth={1.5} />
        <rect x={482} y={708} width={84} height={18} rx={4} fill="#6B1010" stroke="#D4AF37" strokeWidth={1.5} />
        ...
        <rect x={6} y={6} width={588} height={768} rx={8}
          fill="none" stroke="rgba(255,200,60,0.28)" strokeWidth={1.5} />
      </svg>

      {/* ── Logo ── */}
      <div style={{ position:"absolute", top:"18px", left:0, right:0, display:"flex", justifyContent:"center", zIndex:4 }}>
        <div style={{ backgroundColor:"rgba(0,0,0,0.30)", borderRadius:"14px", padding:"6px 22px", border:"1px solid rgba(255,200,60,0.35)" }}>
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img src="/aeonx-logo-light.png" alt="AeonX Digital"
            style={{ height:"44px", objectFit:"contain", display:"block", filter:"drop-shadow(0 0 5px rgba(255,180,40,0.45))" }}
      ...
      <div style={{
        ...
        borderRadius:"10px",
        boxShadow:"0 4px 24px rgba(0,0,0,0.30), 0 0 16px rgba(212,175,55,0.25)",
        overflow:"hidden",
      ...
      {formData.role && (
        <div style={{
          ...
          textAlign:"center", zIndex:4,
          color:"rgba(255,220,180,0.88)", fontSize:"13px", fontWeight:600,
          letterSpacing:"1.5px", textTransform:"uppercase",
        }}>
          {formData.role}
        </div>
      ...
      <div style={{
        ...
        textShadow:"0 2px 10px rgba(0,0,0,0.45), 0 0 30px rgba(255,160,0,0.30)",
        lineHeight:1.1,
      }}>
        Happy Birthday
      </div>
      ...
      <div style={{
        ...
        textAlign:"center", zIndex:4,
        color:"rgba(255,245,220,1)", fontSize:"15px", lineHeight:"1.75", fontWeight:500, fontStyle:"italic",
      }}>
        {message || <span style={{ color:"rgba(255,200,130,0.50)", fontStyle:"italic" }}>Fill in details and generate a message...</span>}
      </div>

      {/* ── Footer ── */}
      <div style={{ position:"absolute", bottom:"24px", left:0, right:0, textAlign:"center", zIndex:4 }}>
        <div style={{ color:"rgba(255,235,200,0.75)", fontSize:"14px" }}>Best Regards,</div>
        <div style={{ color:"rgba(255,220,160,0.95)", fontSize:"16px", fontWeight:700, marginTop:"3px" }}>The AeonX Team</div>
      </div>
    </div>
  );
}
