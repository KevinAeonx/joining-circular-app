...
export default function AnniversaryTemplateEmerald({ formData, message, id }: Props) {
  ...
  return (
    <div id={id} style={{
      ...
      <svg style={{ position:"absolute", inset:0, width:"600px", height:"780px", pointerEvents:"none", zIndex:2 }}
        ...
        {/* Subtle bottom glow */}
        <circle cx={300} cy={780} r={220} fill="rgba(100,200,100,0.08)" />
        ...
        <rect x={6} y={6} width={588} height={768} rx={8}
          fill="none" stroke="rgba(212,175,55,0.28)" strokeWidth={1.5} />
      </svg>

      {/* ── Logo pill ── */}
      <div style={{ position:"absolute", top:"18px", left:0, right:0, display:"flex", justifyContent:"center", zIndex:4 }}>
        <div style={{ backgroundColor:"rgba(0,40,10,0.55)", borderRadius:"14px", padding:"6px 22px", border:"1px solid rgba(212,175,55,0.35)" }}>
      ...
      {formData.role && (
        <div style={{
          ...
          textAlign:"center", zIndex:4,
          color:"rgba(200,255,200,0.90)", fontSize:"13px", fontWeight:600,
          letterSpacing:"1.5px", textTransform:"uppercase",
        }}>
          {formData.role}
        </div>
      ...
      <div style={{
        ...
        color:"#FFD700",
        textShadow:"0 2px 10px rgba(0,0,0,0.35), 0 0 30px rgba(255,200,0,0.25)",
      }}>
        {yearsLabel}
      </div>
      ...
      <div style={{
        ...
        textAlign:"center", zIndex:4,
        color:"rgba(220,255,220,1)", fontSize:"15px", lineHeight:"1.75", fontWeight:500, fontStyle:"italic",
      }}>
        {message || <span style={{ color:"rgba(180,255,180,0.45)", fontStyle:"italic" }}>Fill in details and generate a message...</span>}
      </div>

      {/* ── Footer ── */}
      <div style={{ position:"absolute", bottom:"24px", left:0, right:0, textAlign:"center", zIndex:4 }}>
        <div style={{ color:"rgba(200,255,200,0.70)", fontSize:"14px" }}>Best Regards,</div>
        <div style={{ color:"rgba(200,255,200,0.95)", fontSize:"16px", fontWeight:"bold", marginTop:"3px" }}>The AeonX Team</div>
      </div>
    </div>
  );
}
